...
const createFunc = async (req, res) => {
  try {
    ...
    const productCategoryIds = Array.isArray(categoryId)
      ? categoryId
      : [categoryId];
    ...
    const products = productNames.map((_, index) => ({
      ...
      background:
        backgrounds && backgrounds[index] ? backgrounds[index].buffer : null,
    }));
...
const readFunc = async (req, res) => {
  try {
    if (req.query.page && req.query.limit && req.query.categoryId) {
      ...
      let data = await productApiService.getProductWithPagination(
        +page,
        +limit,
        +categoryId
      );
...
const getAllProducts = async (req, res) => {
  try {
    ...
    if (condition === "ProductInterest") {
      ...
      let data = await productApiService.getAllProductWithCondition(
        condition,
        userId
      );
      return res.status(200).json({
...
const updateFunc = async (req, res) => {
  try {
    ...
    req.body.background = req.files["background"]
